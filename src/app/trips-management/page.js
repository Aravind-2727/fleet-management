...
const getStatusStyle = (status) => {
  switch (status) {
    case 'Assigned':
      return { backgroundColor: 'rgba(124,99,255,0.1)', border: '1px solid rgba(124,99,255,0.25)', color: '#7C63FF' };
    case 'Loading':
      return { backgroundColor: 'rgba(251,146,60,0.1)', border: '1px solid rgba(251,146,60,0.25)', color: '#FB923C' };
    case 'In Transit':
      return { backgroundColor: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.25)', color: '#3B82F6' };
    case 'Unloading':
      return { backgroundColor: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.25)', color: '#A855F7' };
    case 'Delivered':
      return { backgroundColor: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', color: '#22C55E' };
    default:
      return { backgroundColor: 'rgba(107,114,128,0.1)', border: '1px solid rgba(107,114,128,0.25)', color: '#6B7280' };
  }
};
...
export default function TripsManagementPage({ user, onLogout }) {
  ...
  const saveTrip = async () => {
    ...
    setTruckId(''); setDriverId(''); setCustomer('');
    setOrigin(''); setDestination(''); setFreightAmount('');
    setStatus('Assigned'); setShowForm(false);
    fetchTrips();
    setFormLoading(false);
  ...
  const deleteTrip = async (id) => {
    ...
    const { data: trip, error: fetchError } = await supabase
      .from('trips').select('owner_id').eq('id', id).maybeSingle();

    if (fetchError || !trip) { alert('Trip not found'); return; }
    if (trip.owner_id !== authUser.id) { alert('Unauthorized to delete this trip'); return; }
  ...
  const fetchTrucks = async () => {
    ...
    const { data, error } = await supabase
      .from('trucks').select('id, truck_number, status').eq('owner_id', authUser.id);
  ...
  const fetchDrivers = async () => {
    ...
    const { data, error } = await supabase
      .from('drivers').select('id, name, status').eq('owner_id', authUser.id);
        ...
        {showForm && (
          <div style={s.formCard}>
            ...
            <div style={s.field}>
              ...
              <select value={truckId} onChange={(e) => setTruckId(e.target.value)} style={s.input}>
            ...
            <div style={s.field}>
              ...
              <select value={driverId} onChange={(e) => setDriverId(e.target.value)} style={s.input}>
            ...
            <div style={s.field}>
              ...
              <input placeholder="e.g. ABC Logistics" value={customer} onChange={(e) => setCustomer(e.target.value)} style={s.input} />
            </div>

            <div style={s.field}>
              <label style={s.label}>Origin</label>
              <input placeholder="e.g. Mumbai" value={origin} onChange={(e) => setOrigin(e.target.value)} style={s.input} />
            </div>

            <div style={s.field}>
              <label style={s.label}>Destination</label>
              <input placeholder="e.g. Delhi" value={destination} onChange={(e) => setDestination(e.target.value)} style={s.input} />
            </div>

            <div style={s.field}>
              <label style={s.label}>Freight Amount (₹)</label>
              <input type="number" placeholder="e.g. 5000" value={freightAmount} onChange={(e) => setFreightAmount(e.target.value)} style={s.input} />
            </div>
                  ...
                  <tr key={trip.id} style={s.tr}>
                    ...
                    <td style={s.td}>
                      <select
                        ...
                        style={{ ...s.statusSelect, ...getStatusStyle(trip.status) }}
                      >
                        <option value="Assigned">Assigned</option>
                        <option value="Loading">Loading</option>
                        <option value="In Transit">In Transit</option>
